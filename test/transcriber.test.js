...
function makeMockDeepgramRejecting(error) {
  return {
    listen: {
      v1: {
        media: {
          transcribeFile: vi.fn().mockRejectedValue(error),
        },
      },
    },
  };
...
describe("createTranscriber", () => {
  it("returns the transcript text from a successful response", async () => {
    ...
    expect(mockClient.listen.v1.media.transcribeFile).toHaveBeenCalledWith(
      expect.any(Buffer),
  ...
  it("throws when Deepgram returns an error", async () => {
    const mockClient = makeMockDeepgramRejecting(
